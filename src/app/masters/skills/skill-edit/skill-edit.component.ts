...
export class SkillEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.id) {
      this.subscription.push(
        this.requestService.sendRequest(SkillUrls.SINGLE_GET + this.id, 'get', {}).subscribe(res => {
          if (res.status) {
  ...
  formSubmitted($event) {
    ...
    this.subscription.push(
      this.requestService.sendRequest(SkillUrls.UPDATE_PUT, 'put', $event).subscribe(res => {
        if (res.status) {
