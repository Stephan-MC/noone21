  ...
  return (control: AbstractControl): { [key: string]: boolean } | null => {
    if (control.value) {
      ...
      return control.value &&
        parseFloat(moneyMasking(control.value)) <= maxValue
        ? null
        : {
            max: true,
          };
  ...
  return (control: AbstractControl): { [key: string]: boolean } | null => {
    if (control.value || control.value == 0) {
      console.log(
        "control.value",
        control.value &&
          parseFloat(moneyMasking(control.value)).toFixed(2) ==
            parseFloat(minValue).toFixed(2)
          ? { min: true }
          : null
      );
      return parseFloat(moneyMasking(control.value)).toFixed(2) ==
        parseFloat(minValue).toFixed(2) ||
        parseFloat(moneyMasking(control.value)).toFixed(2) <
          parseFloat(minValue).toFixed(2)
        ? {
...
export function ifNotCheckedAlreadyAndSelect(
  ...
  let index = currentPageItems.findIndex(
    (x) => x[matchProp] == currentRow[matchProp]
  );
  let indexInSelected = SelectedItemsArray.findIndex(
    (x) => x[matchProp] == currentRow[matchProp]
  );
...
export function UnCheckAllCheckBoxAndRemoveCurrentUnselect(
  ...
  let index = selectedArray.findIndex(
    (x) => x[matchProp] == currentRow[matchProp]
  );
  if (index > -1) {
...
export function SelectUnCheckItemsOnly(
  currentPageItems,
  SelectedItemsArray,
  matchProp
) {
  ...
  currentPageItems.forEach((element) => {
    let indexInSelected = SelectedItemsArray.findIndex(
      (x) => x[matchProp] == element[matchProp]
    );
    if (indexInSelected == -1) {
...
export function removeCurrentPageSelectionFromSelected(
  currentPageItems,
  selectedArray,
  matchProp
) {
...
export function arrayContainsArray(superset, subset) {
  return _.isEqual(
    _.intersection(superset.sort(), subset.sort()),
    subset.sort()
  );
}
