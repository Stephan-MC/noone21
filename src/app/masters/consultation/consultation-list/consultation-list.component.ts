...
export class ConsultationListComponent implements OnInit {
  ...
  getData(params) {
    this.subscription.push(
      this.requestService.sendRequest(ConsultationUrls.ALL_GET, 'get', params).subscribe(res => {
        if (res.status) {
          this.rows = res.result.data;
          console.log("ConsultationListComponent -> getData -> this.rows ", this.rows)
    ...
    this.ngPopups.confirm('Do you  really want to delete this category?')
      .subscribe(res => {
        if (res) {
          ...
          this.subscription.push(
            this.requestService.sendRequest(ConsultationUrls.DELETE_POST, 'delete_with_body', { ids: delete_params_ids }).subscribe(res => {
              console.log(res);
              if (res && res.status) {
