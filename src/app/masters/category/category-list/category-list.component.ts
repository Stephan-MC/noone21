...
export class CategoryListComponent implements OnInit {
  ...
  getData(params) {
    this.subscription.push(
      this.requestService.sendRequest(CategoryUrl.ALL_GET, 'get', params).subscribe(res => {
        if (res.status) {
  ...
  onAdd() {
    this.router.navigate(["category-add"], { relativeTo: this.route.parent });
  }
    ...
    this.ngPopups.confirm('Do you  really want to delete this category?')
      .subscribe(res => {
        if (res) {
          ...
          this.subscription.push(
            this.requestService.sendRequest(CategoryUrl.DELETE_POST, 'delete_with_body', { ids: delete_params_ids }).subscribe(res => {
              console.log(res);
              if (res && res.status) {
