...
export class LandingPageComponent implements OnInit {
  ...
  byDefaultCategory = [
    { id: "", name: "All Categories", slug: "", shouldNotDisplay: true },
  ];
  ...
  selectItemSubCategory(selectItem) {
    ...
    this.requestService
      .sendRequest(CategoryUrl.CATEGORY_PAGE_ALL, "GET", { is_home: 1 })
      .subscribe(
        (res) => {
          if (res && res.status) {
  ...
  getDoctors() {
    let params = this.makeParams();
    this.requestService.sendRequest(UserUrls.ALL_GET, "GET", params).subscribe(
      (res) => {
        if (res && res.status) {
